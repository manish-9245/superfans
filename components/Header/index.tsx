...
import DropDown from "./buttonAvatar";
import { Menu } from "@/types/menu";
import { menuLoginData, menuNotLoginData } from "./menuData";
/**
...
const Header = () => {
  ...
  useEffect(() => {
    if (data?.id) { 
      setMenuData(menuLoginData);
    } else {
      window.innerWidth >= 768 ? setMenuData(menuLoginData) :
      setMenuData(menuNotLoginData);
    }
  }, [data]);
    ...
    <>
      <header
        className={`header left-0 top-0 z-40 flex w-full items-center bg-transparent ${
          sticky
            ? "!fixed !z-[9999] !bg-white !bg-opacity-80 shadow-sticky backdrop-blur-sm !transition dark:!bg-primary dark:!bg-opacity-10"
            : "absolute"
        }`}
      >
